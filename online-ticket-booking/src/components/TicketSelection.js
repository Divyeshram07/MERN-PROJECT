...
const TicketSelection = ({ eventName }) => {
  ...
  const totalAmount = Object.keys(selectedTickets).reduce(
    (acc, id) =>
      acc + (selectedTickets[id] || 0) * tickets.find((t) => t.id === Number(id)).price,
    0
  );

  const handleProceed = () => {
    const selectedDetails = tickets
          ...
          <div key={ticket.id} className="ticket-card">
            <div className="ticket-info">
              ...
              <p className="ticket-price">
                ₹{ticket.price}{" "}
                {ticket.fastFilling && <span className="fast-filling">| Fast Filling</span>}
              </p>
            ...
            <div className="ticket-actions">
              {selectedTickets[ticket.id] > 0 ? (
                ...
                <button className="add-button" onClick={() => handleAddTicket(ticket.id)}>
                  Add
                </button>
              )}
            </div>
